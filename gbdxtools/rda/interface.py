...
from gbdxtools.rda.graph import VIRTUAL_RDA_URL, register_rda_graph, \
                                ...
                                materialize_status, get_rda_template_metadata
...
class Op(TemplateMeta):
    def __init__(self, name, interface=None):
        super(Op, self).__init__(template_id=None)
        self._operator = name
        self._edges = []
        self._nodes = []
...
class RDA(object):
    def __new__(cls, template_id=None, **kwargs):
        if template_id:
            return TemplateMeta(template_id=template_id)
        else:
            return cls
    # def __getattr__(self, name):
    #     # return Op(name=name, interface=Auth())
    #     return TemplateMeta(template_id=name)
